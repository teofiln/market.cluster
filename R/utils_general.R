hclust_kcca <-
  function(x,
           ...
           weights = NULL,
           group = NULL,
           control = list(dist_method = "euclidean", hclust_method = "complete"),
           simple = FALSE,
           save.data = FALSE)
  {
    # MYCALL <- match.call()
    ...
    z <- flexclust::as.kcca(hc, x, k = k)
    
    if (save.data)
      z@data <- modeltools::ModelEnvMatrix(designMatrix = x)
