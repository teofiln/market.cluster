...
mod_pick_k_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    output$plot_neighborhood <- renderPlot({
      validate(
        need(length(params$km_model)>0,"Run KMeans first")
      )
      plot_neighborhood(params$km_model,k=input$select_k)
    }) 
    ...
    output$plot_hull <- renderPlot({
      ...
      plot_hull(params$km_model,k=input$select_k)
    }) 
    ...
    output$plot_segment_profile <- renderPlot({
      ...
      plot_segment_profile(params$km_model,k=input$select_k)
    })
    ...
    output$plot_segment_stability_within_solution <- renderPlot({
      ...
      plot_segment_stability_within_solution(params$km_model,k=input$select_k)
    ...
    output$plot_gorge <- renderPlot({
      ...
      plot_gorge(params$km_model,k=input$select_k)
    })
    ...
    observeEvent(input$gen_solutions,{
      ...
      params$solutions <<- plot_many_segment_profiles(k=input$select_k, n=input$select_n)
