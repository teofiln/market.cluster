...
plot_flexmix_parameters <- function(fm_model, k){
  ...
  if(length(fm_model)==0) return(NA)
  ...
  parameters(getModel(fm_model,as.character(k))) %>% 
    as.data.frame() %>% 
    rownames_to_column(var="factors") %>%  
    gather(key="cluster", value="probability",-factors) -> tidy_params
  ...
  tidy_params %>% 
    mutate(factors = factor(factors,levels=labels[lab_order])) %>% 
    ggplot(aes(x=factors, y=probability)) + geom_col() + facet_wrap(~cluster) + coord_flip()
